...
const PastHairConditionsScreen = () => {
  ...
  const hairConditions = ['Dryness', 'Breakage', 'Dandruff', 'Thinning', 'Split Ends', 'Oily Scalp'];

  useEffect(() => {
    Animated.parallel([
      Animated.timing(fadeAnim, { toValue: 1, duration: 600, useNativeDriver: true }),
      Animated.spring(translateY, { toValue: 0, friction: 6, useNativeDriver: true }),
      Animated.timing(progressAnim, { toValue: width * 0.75, duration: 800, easing: Easing.out(Easing.ease), useNativeDriver: false }),
    ]).start();
  }, []);
  ...
  const animateSelection = (condition) => {
    ...
    userData.past_hair_condition = selectedConditions.join(', ');
    Animated.sequence([
  ...
  return (
    <LinearGradient colors={['#e8f5e9', '#ffffff']} style={styles.container}>
      ...
      <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY }] }}>
        <Text style={styles.title}>Past Hair Conditions</Text>
        <Text style={styles.subtitle}>Select any previous hair issues or treatments you've had</Text>
      </Animated.View>
      ...
      <View style={styles.confirmButtonContainer}>
        <TouchableOpacity style={[styles.confirmButton, selectedConditions.length === 0 && styles.disabledButton]} activeOpacity={0.9} onPress={familyHistoryHandler} disabled={selectedConditions.length === 0}>
...
export default PastHairConditionsScreen;
