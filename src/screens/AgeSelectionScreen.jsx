...
const AgeSelectionScreen = () => {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.timing(progressAnim, { toValue: width * 0.5, duration: 800, easing: Easing.out(Easing.ease), useNativeDriver: false })
    ]).start();
  }, []);

  const animateSelection = (age) => {
    setSelectedAge(age);
    userData.age = age; // save age to userData
    Animated.sequence([
  ...
  return (
    <LinearGradient colors={['#e8f5e9', '#ffffff']} style={styles.container}>
      ...
      <View style={styles.optionsContainer}>
        {ageGroups.map((age) => (
          <Animated.View key={age} style={{ transform: [selectedAge === age ? { scale: scaleAnim } : { scale: 1 }], opacity: fadeAnim }}>
            <TouchableOpacity style={[styles.option, selectedAge === age ? styles.selectedOption : styles.unselectedOption]} activeOpacity={0.9} onPress={() => animateSelection(age)}>
              <Text style={[styles.optionText, selectedAge === age && styles.selectedText]}>{age}</Text>
            </TouchableOpacity>
          </Animated.View>
        ))}
      </View>
      ...
      <View style={styles.confirmButtonContainer}>
        <TouchableOpacity style={[styles.confirmButton, !selectedAge && styles.disabledButton]} activeOpacity={0.9} onPress={pastConditionsHandler} disabled={!selectedAge}>
...
const styles = StyleSheet.create({
  ...
  optionsContainer: { flex: 1, gap: 20, marginTop: 10 },
  option: { backgroundColor: 'rgba(241, 248, 233, 0.8)', borderRadius: 20, paddingVertical: 20, paddingHorizontal: 25, borderWidth: 1.5, borderColor: '#c8e6c9', shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 8, shadowOffset: { width: 0, height: 3 } },
